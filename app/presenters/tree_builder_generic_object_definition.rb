class TreeBuilderGenericObjectDefinition
  def nodes
    ...
    children = all_generic_object_definitions.collect do |generic_object_definition|
      {
        ...
        :tooltip => generic_object_definition.name,
        :href => "##{generic_object_definition.name}",
        :icon => "fa fa-file-o",
        :tags => ["2"],
        :id   => "god-#{generic_object_definition.id}"
      }
    end

    [{
      :text  => 'Generic Object Definitions',
      :tooltip => 'Generic Object Definitions',
      :href  => '#generic-objects-definitions-root',
      :tags  => ['4'],
      :nodes => children
    }].to_json
  end
